...
pub struct PoseidonConfig {
    pub mds: [[BigInt; STATE_SIZE]; STATE_SIZE],
    pub round_constants: [[BigInt; STATE_SIZE]; FULL_ROUNDS],
...
impl PoseidonConstant {
    pub fn poseidon_config_kimchi_fp() -> PoseidonConfig {
        const BYTE_MDS_0_0: [u8; 32] = [
            3, 121, 156, 103, 172, 31, 234, 183, 231, 145, 100, 125, 48, 86, 10, 226, 96, 78, 155,
            245, 190, 185, 89, 73, 239, 41, 126, 117, 80, 210, 155, 26,
        ];

        const BYTE_MDS_0_1: [u8; 32] = [
            205, 6, 197, 32, 115, 138, 180, 150, 54, 161, 228, 90, 69, 93, 109, 46, 36, 170, 48,
            160, 246, 100, 45, 126, 115, 72, 58, 175, 159, 160, 74, 56,
        ];

        const BYTE_MDS_0_2: [u8; 32] = [
            213, 254, 108, 158, 190, 23, 18, 6, 118, 208, 166, 191, 6, 101, 52, 248, 83, 54, 160,
            167, 228, 92, 77, 6, 128, 48, 188, 9, 2, 123, 43, 61,
        ];

        const BYTE_MDS_1_0: [u8; 32] = [
            66, 58, 191, 180, 243, 170, 183, 36, 62, 145, 165, 168, 164, 104, 152, 167, 190, 171,
            207, 175, 131, 121, 16, 11, 34, 81, 195, 11, 199, 87, 238, 9,
        ];

        const BYTE_MDS_1_1: [u8; 32] = [
            143, 69, 162, 54, 84, 91, 154, 30, 126, 93, 34, 53, 127, 38, 21, 49, 129, 89, 200, 212,
            58, 77, 104, 23, 109, 169, 41, 188, 150, 153, 152, 32,
        ];

        const BYTE_MDS_1_2: [u8; 32] = [
            157, 230, 178, 195, 116, 219, 124, 52, 152, 34, 99, 165, 232, 221, 30, 25, 217, 38,
            127, 238, 25, 100, 17, 50, 226, 26, 23, 46, 219, 154, 227, 20,
        ];

        const BYTE_MDS_2_0: [u8; 32] = [
            203, 103, 239, 22, 35, 17, 127, 53, 148, 122, 89, 65, 212, 213, 82, 84, 139, 129, 33,
            246, 29, 12, 89, 169, 101, 127, 104, 123, 53, 68, 69, 23,
        ];

        const BYTE_MDS_2_1: [u8; 32] = [
            55, 65, 44, 44, 240, 53, 238, 252, 7, 178, 45, 33, 83, 252, 223, 149, 95, 178, 107, 22,
            176, 21, 191, 207, 23, 157, 161, 193, 61, 38, 169, 60,
        ];

        const BYTE_MDS_2_2: [u8; 32] = [
            76, 17, 174, 10, 213, 129, 101, 77, 76, 124, 186, 105, 210, 33, 220, 178, 115, 3, 123,
            11, 248, 18, 168, 183, 99, 171, 212, 117, 239, 251, 241, 60,
        ];

        // Conversion des MDS vectors en arrays
        let mds_data = [
            [
                BigInt::from_signed_bytes_le(&BYTE_MDS_0_0),
                BigInt::from_signed_bytes_le(&BYTE_MDS_0_1),
                BigInt::from_signed_bytes_le(&BYTE_MDS_0_2),
            ],
            [
                BigInt::from_signed_bytes_le(&BYTE_MDS_1_0),
                BigInt::from_signed_bytes_le(&BYTE_MDS_1_1),
                BigInt::from_signed_bytes_le(&BYTE_MDS_1_2),
            ],
            [
                BigInt::from_signed_bytes_le(&BYTE_MDS_2_0),
                BigInt::from_signed_bytes_le(&BYTE_MDS_2_1),
                BigInt::from_signed_bytes_le(&BYTE_MDS_2_2),
            ],
        ];

        // Conversion des round constants vectors en arrays
        let round_constants_data = [
            [
                BigInt::from_str(
        ...
        PoseidonConfig {
            mds: mds_data,
            round_constants: round_constants_data,
...
impl FiniteField {
    ...
    pub fn dot(x: &[BigInt; STATE_SIZE], y: &[BigInt; STATE_SIZE], p: &BigInt) -> BigInt {
...
impl PoseidonHash {
    ...
    pub fn hash(input: Vec<BigInt>) -> BigInt {
        let initial_state = [BigInt::from(0), BigInt::from(0), BigInt::from(0)];
    ...
    pub fn permutation(state: &mut [BigInt; STATE_SIZE], config: &PoseidonConfig) {
        let p = Self::p();
